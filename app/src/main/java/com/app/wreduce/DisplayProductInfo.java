...
public class DisplayProductInfo extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		product =
				MainDatabase.dataBase.where(Product.class).beginsWith("barcodeId", barcodeId).findFirst();
	...
	private void init() {
		((TextView) findViewById(R.id.barcodeId)).setText(barcodeId);
		((EditText) findViewById(R.id.product_name)).setText(product.getProductName());
	...
	public void editProduct(View view) {
		...
		temporary.setProductName(((EditText) findViewById(R.id.product_name)).getText().toString());

		DatePicker expiryPicker = findViewById(R.id.expiration_picker);

		// calendar get date
		temporary.setExpDate(CalendarFunctions.getDateFromPicker(expiryPicker).getTime());
		...
		MainDatabase.dataBase.executeTransaction(
				transactionRealm -> transactionRealm.insertOrUpdate(temporary));
